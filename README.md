...
原理图检查见 `SCHEMATIC_CHECKLISTS.md`，驱动状态机模式见 `DRIVER_PATTERNS.md`，故障案例见 `FAILURE_CASES.md`，波形观察见 `WAVEFORM_GUIDE.md`，测量样例见 `MEASUREMENT_EXAMPLES.md`。

实验记录模板见 `EXPERIMENT_TEMPLATE.md`，接口参数速查见 `PARAMETER_CHEATSHEET.md`，量产测试策略见 `PRODUCTION_TEST.md`，可靠性设计见 `RELIABILITY_GUIDE.md`，回归测试矩阵见 `REGRESSION_TEST_MATRIX.md`，自动化测试治具见 `AUTOMATED_TEST_HARNESS.md`，通信可观测性见 `COMMUNICATION_OBSERVABILITY.md`，发布质量门禁见 `COMMUNICATION_QUALITY_GATE.md`，现场诊断运行手册见 `FIELD_DIAGNOSTIC_RUNBOOK.md`，通信追踪矩阵见 `COMMUNICATION_TRACEABILITY.md`，通信安全与失效安全见 `COMMUNICATION_SECURITY_SAFETY.md`。

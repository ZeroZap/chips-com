...
## 案例 43：50 设备 CPU 过载伪装无线问题——L4 规模天花板 + 树形 vs 纯 Mesh 拓扑

### 现象

50 设备 ZigBee 部署（智能家居 + 智能照明），频繁丢命令 + 3 秒延迟：
- 丢命令率 8%
- 3 秒延迟（命令发出到执行）
- 用户视角："灯经常不听使唤"
- 看似 RF 问题，实际**根因是协调器 CPU 过载**

### 抓包 + 根因（3 大根因）

#### 根因 1：Wiren Board CPU 380-400% 过载

- 协调器用 Wiren Board（Linux SBC + zigbee2mqtt）
- 50 设备 + 多 sensor reporting → MQTT broker + DB 写入
- **CPU 占用 380-400%**（多核打满）
- zigbee2mqtt 队列堆积 → 命令响应延迟 3 秒
- **特征**：sniffer 看 PHY 层完全正常，**延迟在应用层**
- 看似 RF 问题，实际 CPU 瓶颈
...
- 协调器塞在弱电箱角落（被金属面板遮挡）
- 50 设备分布在房子各处
- 边缘设备 RSSI -85 dBm（接近 sensitivity 边界）
- **PHY 层重传率高** + 协调器 CPU 过载 = **延迟 3 秒**

#### 根因 3：纯 Mesh 拓扑在 50 设备规模反而是劣势

- 50 设备都用 router 功能（默认）
- 路由表更新频繁 → CPU 压力 +1
- 实际：3 层树形（1 coord + 3 router + 46 end device）远优于 50 router
...
Step 2：sniffer 看 PHY
  - 抓 PHY 层 RSSI + 重传率
  - PHY 层正常但延迟大 → 排除 RF 物理问题
  - 验证 CPU 是根因

Step 3：物理位置调整
  - 协调器搬到房子中心位置
  - RSSI 平均从 -85 dBm 升到 -65 dBm
  - 重传率从 30% 降到 5%
  - 验证根因 2

Step 4：拓扑改造
  - 改纯 Mesh → 3 层树形（1+3+46）
  - CPU 占用降到 150%
  - 延迟从 3s 降到 200ms
  - 验证根因 3
```
...
```text
方案 A：硬件升级
  - 协调器：Wiren Board → Raspberry Pi 5（4x 性能提升）
  - 验证：CPU 占用从 380% 降到 150%
...
方案 C：拓扑改造
  - 50 router → 3 router + 47 end device
  - 路由表 size 50 → 3
  - 路由更新压力降 90%

方案 D：z2m 升级
  - v2.9.2 → 最新（含 50+ 性能优化）
  - 配置优化：disable 大数组 + enable MQTT retain
  - CPU 占用 -20%
...
- **CPU 过载伪装成无线问题**——50 设备规模**必须看 CPU 占用**
- **协调器物理位置 = 第一根因**——角落放置必败
- **树形比纯 Mesh 稳**（在 50 设备规模）——**路由表 size 与稳定性负相关**
- EFR32MG21（Sonoff Dongle E）比 CC2652P 在拥挤 2.4G 下更稳——**硬件选型影响深远**
- **L4 规模天花板 = 50 设备**——超过必须：
  - 加 router 中继
  - 拆子网
  - 升级协调器硬件
  - 改树形拓扑
- **决策 Checklist**：
  - 50 设备内：树形 + Pi5 + EFR32MG21 + z2m 最新
  - 50-100 设备：拆 2 子网 + 商业协调器
  - 100+ 设备：考虑 Thread / Matter

### 来源

- _Inbox/ZigBee-2026-09-14-candidates.md 候选 2
- sudonull.com（ZigBee 实战社区）
